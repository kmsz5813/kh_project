...
CREATE TABLE SEL_ITEM_STATICS(
		 	...
		 	INSERT INTO SEL_ITEM_STATICS(ID, AC_NAME, SEL_ID, LIKED)   VALUES(SEL_ITEM_STATICS_SEQ.NEXTVAL , '정두', 106, 'N');
SELECT * FROM ACCOUNTS A FULL OUTER JOIN SEL_ITEM_STATICS F ON A.AC_NAME = F.AC_NAME WHERE AC_NUMBER = 125; 	
SELECT * FROM SEL_ITEM;	
SELECT * FROM SEL_ITEM_STATICS;	
DROP TABLE SEL_ITEM_STATICS;


SELECT * FROM SEL_ITEM S FULL OUTER JOIN FILE_UPLOAD ON S.SEL_ID = FILE_BID FULL OUTER JOIN SEL_ITEM_STATICS F ON  S.SEL_ID = F.SEL_ID;
...
CREATE TABLE SEL_ITEM(
			 SEL_ID NUMBER PRIMARY KEY,				-- 판매번호
