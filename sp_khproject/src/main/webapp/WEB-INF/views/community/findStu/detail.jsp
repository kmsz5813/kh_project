...
<body>
	<div id="jb-container">
									...
									<section class="container">
										<div class="mt-3">
											<div class="mb-1 border-bottom border-2 border-secondary">
												<h1>${data.findStu_Title}</h1>
											</div>
											<div class="mb-3">
												<label class="pe-3 text-secondary text-opacity-75">${data.user_Name}</label>
												<fmt:formatDate value="${data.findStu_Date}"
													var="findStu_Date" dateStyle="long" />
												<label class="pe-3 text-secondary text-opacity-75">${data.findStu_Date}</label>
												<label class="pe-3 text-secondary text-opacity-75">조회수:
													${data.findStu_view}</label>
											</div>
											<div class="mb-1 border-bottom border-2 border-secondary">
												<p>${data.findStu_Content}</p>
											</div>
											<div class="mb-1">
												<div onclick="ajaxLike(id_like, ${data.findStu_Id});">
													<i
														class="bi bi-hand-thumbs-up text-secondary text-opacity-50"></i>
													<label id="id_like" class="text-secondary text-opacity-75">${data.findStu_like}</label>
												</div>
											</div>
											
											<div class="mb-1 text-end">
												<c:url var="findStuUrl" value="/community/findStu" />
												<button class="btn btn-primary" type="button"
													onclick="location.href='${findStuUrl}/list'">목록</button>
												<c:if
													test="${data.user_Name eq sessionScope.loginData.ac_name}">
													<button class="btn btn-success" type="button"
														onclick="location.href='${findStuUrl}/modify?id=${data.findStu_Id}'">수정</button>
												...
												<ul class="pagination justify-content-center">
													...
													<c:if test="${commentPage.hasPrevPage()}">
														<li class="page-item"><a class="page-link"
															href="${findStuDetailUrl}&page=${commentPage.prevPageNumber}">Prev</a>
														</li>
													</c:if>
													<c:forEach
														...
														<li
															...
															<a class="page-link"
															href="${findStuDetailUrl}&page=${num}">${num}</a>
														</li>
													</c:forEach>
													<c:if test="${commentPage.hasNextPage()}">
														<li class="page-item"><a class="page-link"
															href="${findStuDetailUrl}&page=${commentPage.nextPageNumber}">Next</a>
														</li>
													</c:if>
												</ul>
											</div>
										...
										<div class="mt-3 mb-3">
													...
													<div class="card border-light">
														<div class="card-header">
															<div class="d-flex justify-content-between">
																<span><small>${comment.user_Name}</small></span> <span><small>${comment.findStu_Date}</small></span>
															</div>
														</div>
														<div class="card-body">
															<input type="hidden" value="${comment.findStu_Id}">
															<c:choose>
																<c:when test="${comment.findStu_isDeleted()}">
																	<p class="text-muted">삭제된 댓글 입니다.</p>
																</c:when>
																<c:otherwise>
																	<c:set var="newLine" value="<%=\"\n\"%>" />
																	<p class="card-text">${fn:replace(comment.findStu_Content, newLine, '<br>')}</p>
																</c:otherwise>
															</c:choose>
															<c:if
																test="${sessionScope.loginData.ac_name eq comment.user_Name}">
																<c:if test="${not comment.findStu_isDeleted()}">
																	<div class="text-end">
																		<button class="btn btn-sm btn-outline-dark"
																			type="button" onclick="changeEdit(this);">수정</button>
																		<button class="btn btn-sm btn-outline-dark"
																			type="button"
																			onclick="commentDelete(this, ${comment.findStu_Id})">삭제</button>
																	</div>
																</c:if>
															</c:if>
														</div>
											...
											<div class="mb-1">
												<form action="${findStuDetailUrl}/comment/add"
													method="post">
													<input type="hidden" name="bid" value="${data.findStu_Id}">
												...
												<div class="modal-content">
													...
													<div class="modal-footer">
														<button type="button" class="btn btn-sm btn-danger"
															...
															onclick="deletefindStuDetail(${data.findStu_Id})">확인</button>
													</div>
												</div>
											</div>
										</div>
		...
		function deletefindStuDetail(findStu_Id) {
			$.ajax({
				url: "${findStuUrl}/delete",
				type: "post",
				data: {
					id: findStu_Id
				},
			...
			$.ajax({
				...
				success: function(data) {
					if(data.code === "success") {
						...
						alert(data.message);
						location.href = "${findStuDetailUrl}";
					}
				}
			});
		}
