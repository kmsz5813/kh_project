...
<body>
	<div id="jb-container">
									...
									<section class="container">
										<div class="mt-3">
											<div class="mb-1 border-bottom border-2 border-secondary">
												<h1>${data.life_Title}</h1>
											</div>
											<div class="mb-3">
												<label class="pe-3 text-secondary text-opacity-75">${data.user_Name}</label>
												<fmt:formatDate value="${data.life_Date}"
													var="life_Date" dateStyle="long" />
												<label class="pe-3 text-secondary text-opacity-75">${data.life_Date}</label>
												<label class="pe-3 text-secondary text-opacity-75">조회수:
													${data.life_view}</label>
											</div>
											<div class="mb-1 border-bottom border-2 border-secondary">
												<p>${data.life_Content}</p>
											</div>
											<div class="mb-1">
												<div onclick="ajaxLike(id_like, ${data.life_Id});">
													<i
														class="bi bi-hand-thumbs-up text-secondary text-opacity-50"></i>
													<label id="id_like" class="text-secondary text-opacity-75">${data.life_like}</label>
												</div>
											</div>
											
											<div class="mb-1 text-end">
												<c:url var="lifeUrl" value="/community/life" />
												<button class="btn btn-primary" type="button"
													onclick="location.href='${lifeUrl}/list'">목록</button>
												<c:if
													test="${data.user_Name eq sessionScope.loginData.ac_name}">
													<button class="btn btn-success" type="button"
														onclick="location.href='${lifeUrl}/modify?id=${data.life_Id}'">수정</button>
												...
												<ul class="pagination justify-content-center">
													...
													<c:forEach
														...
														<li
															...
															<a class="page-link"
															href="${lifeDetailUrl}&page=${num}">${num}</a>
														</li>
													</c:forEach>
										...
										<div class="mt-3 mb-3">
													...
													<div class="card border-light">
														<div class="card-header">
															<div class="d-flex justify-content-between">
																<span><small>${comment.user_Name}</small></span> <span><small>${comment.life_Date}</small></span>
															</div>
														</div>
														<div class="card-body">
															<input type="hidden" value="${comment.life_Id}">
															<c:choose>
																<c:when test="${comment.life_isDeleted()}">
																	<p class="text-muted">삭제된 댓글 입니다.</p>
																</c:when>
																<c:otherwise>
																	<c:set var="newLine" value="<%=\"\n\"%>" />
																	<p class="card-text">${fn:replace(comment.life_Content, newLine, '<br>')}</p>
																</c:otherwise>
															</c:choose>
															<c:if
																test="${sessionScope.loginData.ac_name eq comment.user_Name}">
																<c:if test="${not comment.life_isDeleted()}">
																	<div class="text-end">
																		<button class="btn btn-sm btn-outline-dark"
																			type="button" onclick="changeEdit(this);">수정</button>
																		<button class="btn btn-sm btn-outline-dark"
																			type="button"
																			onclick="commentDelete(this, ${comment.life_Id})">삭제</button>
																	</div>
																</c:if>
															</c:if>
														</div>
											...
											<div class="mb-1">
												<form action="${lifeDetailUrl}/comment/add"
													method="post">
													<input type="hidden" name="bid" value="${data.life_Id}">
												...
												<div class="modal-content">
													...
													<div class="modal-footer">
														<button type="button" class="btn btn-sm btn-danger"
															...
															onclick="deleteLifeDetail(${data.life_Id})">확인</button>
													</div>
												</div>
											</div>
										</div>
		...
		function deleteLifeDetail(life_Id) {
			$.ajax({
				url: "${lifeUrl}/delete",
				type: "post",
				data: {
